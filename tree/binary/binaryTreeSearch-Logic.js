...
class binaryTree{
    ...
    printOdd(root){
        if(root == null) return
        if(root.value % 2==1) console.log(root.value);
        this.printOdd(root.right);
        this.printOdd(root.left)
    }

    // Print Even Number
    printEven(root){
        if(root == null) return
        if(root.value%2==0) console.log(root.value);
        this.printEven(root.left);
        this.printEven(root.right);
    }
